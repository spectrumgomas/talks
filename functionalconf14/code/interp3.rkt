...
(define-type ExprC
  ...
  [letcc (kont : symbol) (body : ExprC)])

;; output (value) type
(define-type Value
  [numV (n : number)]
  [closV (f : (Value (Value -> Value) -> Value))]
  [contV (f : (Value (Value -> Value) -> Value))])
...
(define (interp [expr : ExprC] [env : Env] [k : (Value -> Value)]) : Value
  (type-case ExprC expr
    ...
    [letcc (kont-var b) (interp b 
                                (extend-env (bind kont-var 
                                                  (contV (λ(val dyn-k) (k val))))
                                            env)
                                k)]))

;; tests
(test (interp (appC (lamC 'x (plusC (idC 'x) (numC 2))) (numC 40)) empty (lambda(k) k)) (numV 42))